...
import { navigateToAnyFolder } from './helpers/test-setup';
...
test.describe('Complete Upload Flow', () => {
	...
	test.beforeAll(async ({ browser }) => {
		...
		try {
			// Use a seeded folder - must exist in CI
			testFolderId = await navigateToAnyFolder(page);
			if (!testFolderId) {
				throw new Error('No seeded folder found - ensure test data is seeded before running tests');
			}
